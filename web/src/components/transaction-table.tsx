...
export function TransactionTable({ transactions, compact = false }: { transactions: ExplorerTransaction[]; compact?: boolean }) {
  if (!transactions.length) return <EmptyState title="No transactions yet" detail="Finalized activity will appear here." />
  return (
        ...
        <tbody>
          {transactions.map((transaction) => (
            <tr key={transaction.id}>
              ...
              <td>{transaction.kind === 'claim_reward' ? <AddressLink address={transaction.sender} name={transaction.display_name} /> : transaction.recipient ? <AddressLink address={transaction.recipient} /> : <span className="muted">—</span>}</td>
              <td className="numeric amount-cell">{transaction.kind === 'set_display_name' ? <span className="muted">—</span> : <>{formatKcoin(transaction.amount_atoms)} <small>KC</small></>}</td>
