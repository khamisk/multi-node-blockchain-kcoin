...
KCoin is an experimental test network, not a production currency. This document separates properties backed by automated tests from enforcement visible only in code and from work that is not yet evidenced.
...
- The one-command devnet deliberately derives four public, reproducible development validator keys from source. It demonstrates protocol behavior but is not a validator-authentic deployment: anyone with the repository can reproduce those keys. Byzantine safety claims are backed by the model and isolated tests under the uncompromised-key assumption, not by secrecy of the runnable demo keys.
